...
var Server = function (options) {
    ...
    var conf = {
        functions: {}, host: options.host || '127.0.0.1',
    ...
    *  these functions are missing but planned. */
    if (options.ssl) {
        conf.ssl = {
            ...
            protocol: options.ssl.protocol || 'SSLv3_server_method', clientCert: options.ssl.clientCert || false,
    ...
    var callHandler = function (res, requests) {
        if (requests.length > res.rpc.idx) {
            var error = null; var r = requests[res.rpc.idx];
            res.rpc.idx++;
    ...
    if (conf.ssl) {
        var opts = {
            ...
            handshakeTimeout: conf.ssl.handshakeTimeout || 60, ciphers: conf.ssl.ciphers || 'AES128-GCM-SHA256:RC4:HIGH:!MD5:!aNULL:!EDH',
